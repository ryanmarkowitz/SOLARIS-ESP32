...
{
    if (val & 0x80000) return (int32_t)(val | 0xFFF00000);
    return (int32_t)val;
}
...
esp_err_t solaris_ina228_read_voltage(solaris_ina228_handle_t handle,
                                       float                  *voltage_v)
{
    if (!handle || !voltage_v) return ESP_ERR_INVALID_ARG;
...
esp_err_t solaris_ina228_read_current(solaris_ina228_handle_t handle,
                                       float                  *current_a)
{
    if (!handle || !current_a) return ESP_ERR_INVALID_ARG;
...
esp_err_t solaris_ina228_read_soc(solaris_ina228_handle_t handle,
                                   float                  *soc_percent)
{
    if (!handle || !soc_percent) return ESP_ERR_INVALID_ARG;
    ...
    if (!handle->soc_initialized) {
        ...
        if (clamped > handle->cfg.battery_full_v)  clamped = handle->cfg.battery_full_v;
        if (clamped < handle->cfg.battery_empty_v) clamped = handle->cfg.battery_empty_v;
